...
`just phpt-official-smoke` runs a small PHPT list through both the Rust
PHPT runner and php-src's official `run-tests.php` wrapper.

The Rust runner remains the primary structured runner for PHPT because it
...
```bash
nix develop -c just phpt-official-smoke
```
...
- `PHPT_MANIFEST=tests/phpt/manifests/original-smoke.jsonl`;
