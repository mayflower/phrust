...
| `standard.url-html` | 47 / 5 | 26 PASS | 0 / 0 | Complete HTML entity tables, non-default charsets/flags, object query encoding, parse_url edge cases beyond the basic corpus, remaining parse_str edge cases including startup filter.default deprecation output, and broader URL behavior remain outside the selected gate. |
