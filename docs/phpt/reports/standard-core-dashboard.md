...
| `standard.url-html` | 31 / 5 | 42 PASS | 0 / 0 | Complete HTML4/HTML5/XHTML entity tables, non-default charsets beyond selected UTF-8 and SJIS basic-entity coverage, object query encoding/property visibility, parse_url edge cases beyond the selected corpus, remaining parse_str edge cases including startup filter.default deprecation output, and broader URL behavior remain outside the selected gate. |
