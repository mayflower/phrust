...
- Selected fixtures:
  ...
  - selected `ext/hash/tests/*.phpt` rows for adler32, CRC/FNV/JOOAT, MD2/MD4,
    md5, RIPEMD, sha1, SHA-2/SHA3, HMAC-md5, file hashing, HKDF, PBKDF2, and
    stream updates
...
`ripemd256`, and `ripemd320`; digest-only coverage also includes `adler32`,
`crc32`, `crc32b`, `crc32c`, `fnv132`, `fnv1a32`, `fnv164`, `fnv1a64`, and
`joaat`.

The selected rows cover SHA-256 file digests, SHA-256 file HMACs, raw binary
file digest output, upstream adler32/CRC/FNV/JOOAT/MD2/MD4/md5/RIPEMD,
sha1/SHA-2/SHA3 digest vectors, HMAC-md5, upstream file hashing,
...
incremental SHA-256 hashing, copying a partially updated context, HMAC
contexts, and finalized context rejection.

## Gaps

The full php-src hash algorithm inventory remains out of scope for this slice:
`xxh*`, `murmur*`, `haval*`, `tiger*`, `snefru*`, `gost*`, and `whirlpool`
are still unsupported. HashContext magic serialization/debug-info parity is
also not complete.
...
After adding Adler-32, CRC32/CRC32C, FNV, JOAAT, SHA3, RIPEMD, MD2, and MD4,
the selected manifest contains 33 green rows.
