...
Prompt 06.23 implements the PHP error-handling MVP in the VM because the
...
- Uncaught VM exceptions call the active exception handler before script
  termination.
- Direct calls to standard builtins that return Phase 6 TypeError or ValueError
...
- Differential fixtures cover simple PHP-compatible handler and mask behavior:
  - `PHASE6_STDLIB_ERROR_HANDLING`
  - `PHASE6_STDLIB_ERROR_REPORTING`
  - `PHASE6_STDLIB_EXCEPTION_HANDLER`
  - `PHASE6_STDLIB_TYPE_VALUE_ERRORS`
