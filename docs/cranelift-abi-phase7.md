# Phase 7 Cranelift Array ABI

Date: 2026-06-23.

Reference target: PHP 8.5.7 (`php-8.5.7`).

Prompt 07.CL.20 defines the read-only packed-array ABI surface used by later
...
must not read it directly in Phase 7. The only valid packed-array read surface
is the helper API below, which uses the public `PhpArray` facade.

## Layout Version And Guards

`php_runtime::PHP_JIT_ARRAY_LAYOUT_VERSION` is the stable Phase 7 layout token
...
Prompt 07.CL.20 does not lower array access to Cranelift. Prompt 07.CL.21 adds
...
Prompt 07.CL.22 reuses the same read-only helper boundary for packed foreach
...
Prompt 07.CL.26 introduces the first object property fast path through a
...
The Phase 7 property-load ABI is read-only. It does not expose property writes,
...
At Prompt 07.CL.20 both fixtures are expected to preserve output parity through
the existing fallback path. Native packed-array fetch counters are introduced
by Prompt 07.CL.21, which adds:
...
Prompt 07.CL.22 adds packed-foreach reduction fixtures:
