# Phase 5 Object Semantics

This document tracks the Phase 5 object-model contract as it grows from the
Phase 4 object MVP. The runtime still consumes the single frontend pipeline:
`php_lexer` -> `php_syntax` -> `php_ast` -> `php_semantics` -> `php_ir` ->
`php_runtime` -> `php_vm`.

## Prompt 16 Scope

Prompt 16 adds the first executable class hierarchy layer:
...
Prompt 18 extends call frames with explicit `scope_class`, `called_class`, and
...
Implemented Prompt 16 visibility checks:
...
Prompt 16 finalizes inherited instance property slots at object construction so
parent-declared properties are present on child instances. Prompt 17 extends
...
Prompt 18 adds executable late static binding for static and instance method
...
Prompt 20 adds runtime-visible interface metadata to IR and VM class entries.
...
class-like parameter, return, and property type checks. Prompt 29 extends the
...
Prompt 29 keeps exceptions as VM metadata objects instead of full userland
...
Prompt 25 adds executable enum metadata and runtime singleton cases for the
...
Prompt 26 exposes enum-case attribute metadata through the Reflection metadata
...
Prompt 26 carries Phase 3 attribute metadata into IR and runtime class tables
...
Prompt 27 broadens the VM Reflection metadata MVP for framework-style smoke
queries, and Prompt 28 adds enum, closure, and structured callable reflection:
...
Prompt 28 MVP supports closures and user-function callables; method, internal
...
Prompt 21 adds property overloading for instance property operations:
...
Prompt 24 adds executable PHP 8.4+ property hooks for the fixture-covered
runtime paths. The parser emits `PROPERTY_HOOK_DECL` nodes, Phase 3 HIR records
...
Prompt 22 adds method and object magic for the fixture-covered runtime paths:
...
Prompt 23 completes the fixture-covered clone path:
...
- private/protected/readonly/static replacement attempts outside the Prompt 24
...
Prompt 30 introduces a VM-owned shutdown `DestructorQueue` rather than running
...
Those items remain tracked in `docs/phase5-known-gaps.md` and later Phase 5
prompts.

## Public API Surface

Phase 6 should reuse these runtime metadata APIs:
...
and repeated enum-case/static-property access. Phase 6 may cache those paths,
