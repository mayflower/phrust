# Runtime Reference Diff

Runtime reference diffing compares Rust VM fixture execution with the pinned PHP
...
`target/runtime/runtime-diff`. `phpt-smoke` runs the selected local PHPT smoke
fixtures through the Rust VM.

## Fixture Metadata

Runtime fixtures are discovered under `fixtures/runtime/**/*.php`. Metadata is
read from initial comments with the `runtime-runtime:` prefix and may define:
...
Generated files under `target/runtime/` are not committed.

## Typical Deviations

Expected Runtime deviations from PHP reference behavior include:
...
`docs/runtime-known-gaps.md` with an ID, example or fixture, and Runtime semantics+
assignment.
