# Phase 7 JIT Experiment

Phase 7 treats JIT work as an experiment behind a default-off feature. The
decision record is `docs/adr/0076-cranelift-jit-experiment.md`.

## Decision Summary

The project may explore Cranelift in Phase 7, but only as a conservative
...
Eligible Phase 7 regions are limited to:
...
must be isolated and documented in `docs/safety-audit-phase7.md`.
...
The Phase 7 code cache is request-local. Cache keys include IR identity,
...
Prompt 07.53 int-leaf prototype through safe VM-owned code after Cranelift IR
...
Prompt 07.54 routes JIT attempts through the request-local tiering policy.
...
Prompt 07.49 adds `crates/php_jit` as a default-off API skeleton. Prompt 07.50
adds conservative eligibility analysis for a primitive int/bool leaf-function IR
subset with stable rejection and unknown reason codes. Prompt 07.51 adds a
safe, handle-based VM/JIT ABI for context/frame views, value boundaries,
bailout/deopt results, runtime callouts, and exception markers. Prompt 07.52
...
Prompt 07.53 adds the first execution integration under both
...
Prompt 07.54 adds the tiering policy and stats surface. The JIT remains
...
Phase 7 implementation does not allocate executable memory and is not
production-ready native JIT.

## Optional 07.F W^X Status

Optional prompt 07.F is conditional: build a W^X/mprotect prototype only if the
Phase 7 JIT actually executes native code. That precondition is false in the
...
Because no executable memory exists in Phase 7, adding a dummy mprotect wrapper
...
- update `docs/safety-audit-phase7.md` before any native entry is enabled.
