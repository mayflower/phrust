# Phase 7 Quickening And Inline Caches

This document is the implementation guide for the model accepted in
`docs/adr/0074-quickening-inline-cache-model.md`. It describes how Phase 7
...
Prompt 07.54 adds a request-local tiering controller above quickening, inline
...
```bash
php-vm run --tiering-stats-json target/phase7/tiering.json script.php
```
...
Prompt 07.43 unifies quickening and inline-cache miss handling through the
...
The implemented Phase 7 side-table specialization is `ADD_INT_INT`. The
...
The implemented Phase 7 side-table specialization is `CONCAT_STRING_STRING`.
...
The implemented Phase 7 side-table specialization is `PACKED_ARRAY_INT_KEY`.
...
The implemented Prompt 07.30 function-call IC is monomorphic and
...
The implemented Prompt 07.31 method-call IC is monomorphic and
...
The implemented Prompt 07.32 property-fetch IC is monomorphic and
...
The implemented Prompt 07.33 class/static IC is monomorphic and
...
`docs/adr/0075-cache-invalidation-model.md`. Phase 7 cache entries are
...
Phase 7 does not persist quickening state globally. Cross-request or shared
...
entry may become polymorphic. Phase 7 caps polymorphic entries at four arms.
...
behavior unless the prompt explicitly introduces adaptive thresholds tied to
those counters.

## Validation Requirements

Each specialization prompt must include:
...
The initial framework prompt kept specialization conservative while installing
the state machine and stats. Current Phase 7 smoke gates exercise concrete
...
nix develop -c target/debug/php-vm run \
  --quickening=on \
  --inline-caches=on \
  --counters-json target/phase7/adaptive-counters.json \
  tests/fixtures/phase7/perf_smoke/function_calls.php
```
...
nix develop -c target/debug/php-vm run \
  --quickening=on \
  --tiering=off \
  tests/fixtures/phase7/perf_smoke/arithmetic.php
```

## Troubleshooting

- Output differences: rerun the same fixture with `--quickening=off` and
  `--inline-caches=off`, then inspect `target/phase7/quickening-smoke` or
  `target/phase7/inline-cache-smoke` artifacts for the first differing mode.
