# Runtime Runtime Reference Oracle

Runtime runtime comparisons use the pinned PHP `8.5.7` CLI through the
...
documented in `docs/runtime-reference-diff.md`.
...
command, then writes reports under `target/runtime/runtime-diff`. The runner
discovers `fixtures/runtime/**/*.php`, applies optional `runtime-runtime`
...
`skipped`, and `known_gap` counts. Files under `target/runtime/` are generated
...
trace and IR debug files under `target/runtime/failures/`; that directory is a
...
because Runtime has no per-test INI model. Other PHPT sections are skipped
...
`phpt-smoke-report.json` are written under `target/runtime/phpt-smoke`.

Standard library adds `just extension-phpt-smoke` for selected upstream extension PHPTs.
That gate reads `fixtures/stdlib/phpt_extension_manifest.toml`, references
...
`target/stdlib/phpt-extension-smoke`.
