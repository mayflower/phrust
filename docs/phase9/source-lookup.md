# Phase 9 Source Lookup

The Phase 9 source symbol index is a navigation aid for php-src behavior
...
```bash
nix develop -c just phpt-source-lookup-phase9 SYMBOL=strlen
nix develop -c just phpt-source-lookup-phase9 SYMBOL=ArrayObject::offsetGet
```
...
and concise source notes under `docs/phase9/php-src-behavior/`.

Regenerate the index together with the source hash manifest:

```bash
nix develop -c just phpt-source-index-phase9
```
