...
The pinned php-src checkout is read-only input. Phase 9 tools may read Original
...
The preferred Phase 9 source path is:
...
Commands may override the path with `PHP_SRC_DIR`. A future source-index prompt
will create `tests/phpt/manifests/php-src-hashes.jsonl` with one JSONL entry per
tracked PHPT or source file.

## Hash Verification

Once the manifest exists, `just phpt-verify-source-integrity-phase9` verifies:
