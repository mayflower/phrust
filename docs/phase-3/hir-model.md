...
The Phase 3 HIR is the semantic frontend's structured, non-lossless
...
- `hir::decl`, `hir::expr`, `hir::stmt`, `hir::types`, `hir::attributes`, and
  `hir::names` contain records that later lowering prompts fill without
  introducing runtime values.
- `hir::expr` and `hir::stmt` now contain structural expression and statement
  HIR for Prompt 16, including recovery-safe `Missing` nodes and PHP 8.5
  pipe/clone-with/first-class-callable forms.
- `hir::const_expr` records Prompt 17 constant-expression candidates with
...
file and maps its `ModuleId` to the CST root range. Later prompts are
...
`docs/phase-3/expression-statement-hir.md` documents the Prompt 16 expression
