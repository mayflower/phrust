...
Phase 4 should start only after Phase 3 provides a stable semantic frontend
contract.
...
- Treat `semantic_diagnostics` as a pre-bytecode gate. Error diagnostics mean
  Phase 4 should not lower executable code for that file.

## Risks Carried Forward

- PHP references and copy-on-write behavior are not modeled in Phase 3.
- Include/eval effects are deferred metadata only. Phase 4 must decide how to
