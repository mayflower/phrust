...
- Reference commit observed by `verify-phase0`:
  `35eab8c08bc590758d05813b0ff7a3d8c3e67b79`
- Project base commit during audit: `0146f39`
- Worktree state: dirty, with Phase 2 and Phase 3 implementation files staged
...
Implemented the Phase 3 semantic frontend layer:
...
- Phase 3 has a dedicated `php_ast` layer instead of adding typed views to
  `php_syntax`.
- Phase 3 has a dedicated `php_semantics` layer for HIR, declarations, scopes,
...
- Semantic diagnostic IDs are documented in
  `docs/phase-3/compile-time-diagnostics.md`.
- Phase 0, Phase 1, and Phase 2 verification commands remain available and pass.
- `nix develop -c just verify-phase3` is the local and CI Phase 3 hard gate.
- Phase 3 snapshots and semantic reference diff are deterministic over the
  curated fixture set.
...
- Exact PHP error message wording compatibility is incomplete; Phase 3 compares
...
- Runtime value behavior, references, copy-on-write, VM, bytecode, JIT,
  extensions, and Zend ABI emulation are outside Phase 3 scope.
...
- `just verify-phase0`: pass
- `just verify-phase1`: pass
- `just verify-phase2`: pass
- `just verify-phase3`: pass
...
- Parser CST/diagnostic snapshots remain covered by Phase 2 gates
...
- Include/eval: Phase 3 records deferred metadata but does not execute files or
...
- Exact error-message compatibility: Phase 3 has stable IDs but does not
  guarantee byte-for-byte PHP fatal error messages.
- References/copy-on-write: not modeled in Phase 3 and must not be inferred from
  HIR alone.
- CFG checks: full `goto` boundary validation and more precise control-flow
  legality belong in Phase 4.

## Phase 4 Handoff

Start Phase 4 from `php_semantics::query::frontend::analyze_file`.
...
- The Phase 3 GitHub workflow runs the same command as local verification:
  `nix develop -c just verify-phase3`.
- `verify-phase3` does not run corpus, fuzz, or benchmark jobs by default.
- Soft targets are explicit:
  `semantic-corpus-smoke`, `fuzz-frontend-smoke`, and `bench-frontend`.

## Final Assessment

Phase 3 is implemented and verified for the bounded semantic frontend scope.
The remaining gaps are documented, fixture-backed, and assigned to later phases
