...
Phase 3 is a read-only consumer of the existing source, token, and CST layers.
The pipeline is:
...
current implementation vertical. See `docs/phase-3/incremental-frontend-api.md`.

`php_frontend_cli symbols <file>` prints the Phase 3 declaration table. The
...
PHP permits declarations inside statement contexts such as `if` blocks. Phase 3
...
Phase 3 unless a fixture explicitly asserts it.
...
dispatch, and extension behavior are Phase 4 or later concerns.
...
in `docs/phase-3/php85-semantics.md`.
