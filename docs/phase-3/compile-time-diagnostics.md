...
Exact PHP error text compatibility is not required for Phase 3. Acceptance,
diagnostic category, source location, and known-gap classification are the
primary comparison points.

## Prompt 07 Diagnostic System
...
| --- | --- | --- | --- |
| `PHS0000` | note | any semantic phase | Reserved marker for future semantic diagnostics |
...
| `E_PHP_DUPLICATE_DECLARATION` | error | declaration collection | Phase 3 same-file duplicate declaration check |
...
| `W_PHP_REFERENCE_BEHAVIOR_DEFERRED` | warning | any semantic phase | reference behavior exists but is intentionally deferred |
| `N_PHP_RUNTIME_CHECK_DEFERRED` | note | any semantic phase | runtime-only behavior intentionally deferred to later phases |

The mapping is intentionally coarse. Phase 3 tracks stable local IDs and source
