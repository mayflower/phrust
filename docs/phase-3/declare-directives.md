...
Prompt 27 records `declare(...)` statements as frontend metadata. The semantic
...
The Phase 3 checker only reports reference-confirmed compile-time failures:
...
`strict_types` runtime coercion behavior is deferred to Phase 4. `ticks` does
