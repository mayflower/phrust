...
`php_ast` provides typed views over the Phase 2 CST. An AST view is a thin,
...
## Prompt 04 Declaration and Member Views
...
- `ParameterList` as the prompt-facing alias for `ParamList`.
...
`PropertyItem`, and `EnumCase` reflect current Phase 2 CST shapes. They do not
invent separate parser nodes or perform semantic validation.

## Prompt 05 Statement, Expression, and Type Views
...
Prompt-facing compatibility names are provided where the CST intentionally uses
