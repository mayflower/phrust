...
Lexer treats PHP tokenizer constants as reference data, not Rust numeric
...
Use `token_get_all($code, 0)` for the hard Lexer oracle:
...
the Lexer hard gate because it can depend on parser context.
...
The script writes `docs/lexer/token-coverage.generated.md` from the pinned PHP
...
documented work rather than hard Lexer gates.
...
byte-oriented. `TOKEN_PARSE` keyword relaxation is documented as Parser work.
