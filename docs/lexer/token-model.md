...
Lexer tokens are normalized for comparison with PHP `8.5.7`
...
The Lexer token surface must explicitly include:
...
Lexer hard-gates `token_get_all($code, 0)` so token classification remains a
pure lexical pass:
