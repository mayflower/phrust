...
The Lexer lexer is byte-oriented and lossless. PHP source is lexically
...
byte columns; Unicode grapheme width is outside Lexer.

## Crate Layout

Lexer introduces `crates/php_lexer` as the lexer library and
...
equivalence is parser-contextual and belongs to Parser.
...
allowlist option for future temporary exceptions, but Lexer does not accept
...
Lexer intentionally avoids `unsafe`, SIMD, interning, arenas, and advanced
