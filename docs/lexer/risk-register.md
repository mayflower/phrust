...
| --- | --- | --- |
| Accidentally implementing parser behavior in the lexer. | Layer boundary becomes unclear and `TOKEN_PARSE` parity is misrepresented. | Keep hard gate to `token_get_all($code, 0)` and document parser-contextual cases for Parser. |
...
| `TOKEN_PARSE` behavior is overclaimed. | Lexer claims parser-contextual compatibility it cannot know. | Prepare the flag but defer strict `TOKEN_PARSE` parity to Parser. |
