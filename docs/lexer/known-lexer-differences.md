...
Lexer has strict Rust-vs-PHP fixture comparison for the curated lexer
fixtures.
...
mismatch, and the Lexer or Parser issue that will remove the exception. The
default Lexer gate must remain strict.
...
- Add byte-exact invalid-input handling if Parser needs non-UTF-8 source
  preservation.
