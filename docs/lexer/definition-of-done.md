# Lexer Definition of Done

Lexer builds only the PHP lexer/tokenization layer for the pinned PHP
...
- `TOKEN_PARSE` support is prepared and documented, but not a hard Lexer
...
`T_INLINE_HTML` token spanning the full source for non-empty input. Later Lexer
work replaces that behavior with reference-compatible tokenization.

## Reference Gate

When a reference PHP binary is available through `REFERENCE_PHP` or
`third_party/php-src/sapi/cli/php`, Lexer also runs:
