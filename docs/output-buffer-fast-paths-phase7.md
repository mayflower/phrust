# Phase 7 Output Buffer Fast Paths

Prompt 07.42 keeps PHP-visible output semantics unchanged while reducing
...
Output buffering callbacks remain the Phase 6 unsupported gap. Fast paths must
...
callback unsupported diagnostic. The phase7 smoke corpus includes
`tests/fixtures/phase7/perf_smoke/output_writes.php` so `bench-phase7-smoke`
