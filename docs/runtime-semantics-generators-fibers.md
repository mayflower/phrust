# Runtime semantics Generators and Fibers

Runtime semantics models generators and fibers as runtime objects owned by
`php_runtime` and resumed by `php_vm`. They consume Semantic frontend HIR and Runtime IR;
...
Runtime semantics generator objects and forwards yielded keys, yielded values, and the
...
- delegation to Iterator/SPL objects is outside Runtime semantics;
...
- exact Zend diagnostics, destructor timing, and all exception/finally
  interaction edges are Standard library+ work.
...
Standard library code should treat these Rust APIs as the current integration surface:
...
The continuation structs inside `php_vm` are intentionally private. Standard library may
...
simple and deterministic, but it is a performance-critical area for Standard library
...
Suspended stacks must remain GC roots. Runtime semantics has explicit root categories for
generator and fiber stacks; Standard library must keep those roots precise when adding
