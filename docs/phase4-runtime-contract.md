...
Phase 4 turns the Phase 3 semantic frontend into an executable MVP:
...
Phase 4 introduces a versioned, source-mapped IR, a deterministic interpreter
VM, a minimal runtime value model, and a CLI that can compile, dump, execute,
and compare small PHP programs. The only regular input to Phase 4 is the Phase
3 frontend output, preferably through
`php_semantics::query::frontend::analyze_file`.

Phase 4 may execute only files that pass parser and semantic frontend checks.
...
The Phase 4 MVP should execute:
...
| `fixture` | Fixture path or `planned` until the fixture prompt creates it |
...
| `resolution_phase` | `phase4`, `phase5`, or a narrower named phase |
...
It is separate from the mutable builtin execution context. The Phase 4 VM uses
...
Prompt 19 introduces `php_runtime::RuntimeDiagnostic` as the stable diagnostic
...
`ValueSlot` provide the Prompt 23 local-alias scaffold without exposing
...
Prompt 23 `BindReference` instruction can bind two local slots to one
...
Prompt 12 control flow uses explicit basic blocks. `JumpIfTrue` targets the
truthy branch and uses the next physical block as the false fallthrough;
`JumpIfFalse` remains available for hand-authored IR. Prompt 13 adds `JumpIf`
...
Prompt 14 executes named user functions from top-level scripts. HIR function
...
Prompt 15 adds required/optional parameter metadata, folded scalar/null/string
defaults, simple variadics, argument-count runtime diagnostics, and return-type
MVP checks. Omitted optional parameters are filled only from Phase 3
...
and nullable wrappers are lowered into the runtime type adapter. Prompt 30 uses
that Phase 3 type information for scalar/nullable parameter checks,
...
Prompt 16 executes simple closure values through the existing call-frame
...
Prompt 17 introduces a narrow runtime callable model. Simple first-class
...
Prompt 16 are callable through the same helper. Method callables and unresolved
...
Prompt 18 moves internal functions into `php_runtime::BuiltinRegistry`. The
...
standard function in this prompt.
...
Prompt 25 implements a root-constrained local `IncludeLoader` in `php_vm`.
...
Prompt 26 introduces runtime `ObjectRef` values backed by encapsulated object
...
Phase 3 class declarations lower into `IrUnit.classes`; `CompiledUnit` exposes
...
command. `run <path.php>` reads the file, invokes the Phase 3 frontend, lowers
...
Phase 4 prioritizes determinism, source maps, diagnostics, and differential
