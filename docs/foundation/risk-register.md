...
| Scope creep into lexer or parser work. | Foundation work becomes hard to review and validate. | Keep Foundation limited to environment, reference, docs, scripts, and skeletons. |
...
| Missing test oracles. | Later implementation can pass local tests while diverging from PHP. | Document token, parse, runtime, and framework smoke oracles in Foundation. |
...
| `foreach` mutation behavior is complex. | Array iteration can diverge from PHP in common code. | Build targeted `.phpt`-derived fixtures in later layers. |
...
| Streams, resources, and extensions are broad. | Runtime compatibility scope can expand quickly. | Stage extension coverage and keep Foundation boundaries explicit. |

## Review Cadence

This register should be updated whenever a Foundation decision changes or a later
layer discovers a new compatibility, reproducibility, or provenance risk.
