...
Foundation fixes the compatibility target to PHP `8.5.7`, Git tag
`php-8.5.7`, from:
...
The optional build enables CLI and tokenizer support so later layers can use
`token_get_all()` as a lexer oracle.

## Critical Reference Files

The Foundation reference contract tracks these critical files:
