# Phase 7 Shared Cache Research Spike

Prompt 07.C investigates future shared-cache options for SAPI or daemon phases.
Phase 7 does not add production shared memory code.

## Options

| Option | Scope | Benefits | Risks |
| --- | --- | --- | --- |
| Disk bytecode cache | Existing Phase 7 request/process boundary | Simple invalidation, easy inspection, no shared mutable memory | Filesystem latency, per-process reads, dependency invalidation still conservative |
...
Phase 8 should prototype immutable mmap reads over the existing disk cache
format before attempting mutable shared memory. Phase 9 or a dedicated SAPI
phase can evaluate a shared arena after daemon lifecycle, preload semantics,
