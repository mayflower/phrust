# Phase 7 Optimizer Passes

Phase 7 optimizer work is a correctness-preserving IR rewrite layer. It runs
...
nix develop -c target/debug/php-vm dump-ir tests/fixtures/phase7/optimizer/arithmetic.php
nix develop -c target/debug/php-vm run --opt-level=0 tests/fixtures/phase7/optimizer/arithmetic.php
nix develop -c target/debug/php-vm run --opt-level=1 tests/fixtures/phase7/optimizer/arithmetic.php
nix develop -c target/debug/php-vm run --opt-level=2 tests/fixtures/phase7/optimizer/arithmetic.php
```

`--opt-level=0` skips the optimizer pipeline. `--opt-level=1` and
`--opt-level=2` run the Phase 7 pass pipeline and are tested against the
baseline by `optimizer-diff`, `regression-phase7`, and `perf-flag-matrix`.
...
- If `perf-compare` does not show a speedup, do not treat that as an optimizer
  correctness failure. Phase 7 wall-clock budgets are advisory.
