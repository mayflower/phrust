# Phase 5 Unsafe Audit and Hardening

Phase 5 touches reference cells, Copy-on-Write arrays and strings, GC debug
...
As of this Phase 5 audit, there are no Rust `unsafe` blocks, `unsafe fn`
...
runtime or VM fails `verify-phase5`.
...
- `crates/php_vm/src/vm.rs` allows `clippy::too_many_arguments` for VM helper
  ...
  to hiding mutable VM state in broader global abstractions during Phase 5.
...
The reproducible Phase 5 devshell must expose:
...
`nix develop -c just phase5-toolchain-audit` checks those requirements. The
target intentionally avoids host-specific absolute paths; the shell may use the
developer's `PATH` for optional tools, but required Phase 5 validation tools
...
part of CI or `verify-phase5`.

Run:

```bash
nix develop -c env PHRUST_RUN_SANITIZER=1 just phase5-sanitizer-smoke
```
...
Prompt D keeps the runtime prototype mode unsafe-free by comparing core public
...
unit tests, not a replacement for the Phase 5 fixture or differential gates.

## CI Artifacts

The Phase 5 CI workflow uploads fixture and diff reports from `target/phase5`
and runtime smoke reports from `target/phase4` after the gate. These artifacts
