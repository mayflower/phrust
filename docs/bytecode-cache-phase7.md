# Phase 7 Bytecode Cache Handoff

Phase 7 bytecode-cache work starts from
...
| Field | Required Phase 7 behavior |
| --- | --- |
...
misses unless later prompts add precise dependency tracking.

## Request and Cache Lifecycle

The Phase 7 cache lifecycle is request-local at execution time and disk-backed
...
flowchart TD
    ...
    Fingerprint --> Lookup["lookup target/phase7/bytecode-cache/*.phbc"]
...
Phase 7 does not implement a daemon, worker pool, shared-memory cache, FPM SAPI,
or production request manager. Later phases can reuse the same artifact and
...
- Keep Phase 7 disk artifacts optional and safely ignorable.
...
Phase 7 can cache simple source files that have no dynamic include dependency
...
`eval` creates source text at runtime. Phase 7 cache artifacts must not claim
...
- Do not serialize eval-generated IR into a file artifact for the enclosing
  source.
- Do not treat eval strings as stable dependencies unless a later phase defines
...
PSR-4 prefix maps, and filesystem layout. Phase 7 treats autoload-sensitive
...
The Phase 7 cache is deliberately not a production PHP OPcache replacement.

- No shared-memory OPcache manager is required in Phase 7.
- No preloading support is required in Phase 7.
- No production SAPI lifecycle, FPM worker manager, or cross-request shared
  runtime state is required in Phase 7.
- No Zend ABI, extension persistence, interned-string table, class-entry sharing,
  or host resource persistence is required in Phase 7.
...
nix develop -c target/debug/php-vm run \
  ...
  --bytecode-cache-dir target/phase7/manual-cache \
  --bytecode-cache-stats \
  tests/fixtures/phase7/bytecode_cache/simple.php
```
...
- Include/require invalidation, Composer/autoload dependency tracking,
  ...
  Phase 8-or-later handoff topics unless a later Phase 7 prompt explicitly adds
  tested support.
