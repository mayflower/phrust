...
Performance performance artifacts are generated under `target/performance` and are not
committed because wall-clock timings are host-specific. This document records the
latest local snapshot and the conservative budgets used for Performance gates.
...
| Date | 2026-06-23 |
| Commit | `81ac9ae6b32b` |
| Worktree | Dirty local Performance implementation worktree; use as development evidence, not a release baseline. |
...
| Baseline artifact | `target/performance/baseline.json` |
| Current artifact | `target/performance/benchmark-smoke.json` |
| Comparison artifact | `target/performance/perf-compare.md` and `target/performance/perf-compare.json` |
| Report artifact | `target/performance/perf-report.md` and `target/performance/perf-report.json` |
...
The benchmark smoke covers these deterministic Performance scenarios for both
...
The Work item correctness matrix separately compared 18 fixtures across 6
...
Performance budgets are intentionally conservative:

- Correctness is mandatory. `verify-performance`, `performance-regression`, and
...
- Callgrind is optional and platform-gated. When `callgrind-smoke`
...
Use `target/performance/perf-compare.json` for machine-readable comparison data and
`target/performance/perf-report.md` for the generated local report.
