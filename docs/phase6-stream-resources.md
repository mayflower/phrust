# Phase 6 Stream Resources

Reference target: PHP 8.5.7 (`php-8.5.7`).

Prompt 06.26 adds the runtime resource handle model used by later stream and
filesystem prompts. Prompt 06.27 adds the bounded wrapper registry for local
...
The Phase 6 GC tracks arrays, objects, references, closures, fibers, and
generators for cycle diagnostics; resources do not add graph edges and are
finalized through `ResourceTable::finalize_all()`.

Later file, directory, and wrapper prompts should call `finalize_all()` during
...
Prompts 06.26 and 06.27 intentionally do not add PHP-visible `fopen`,
`fread`, `fwrite`, directory handles, or stat/path functions. Those PHP-visible
producers and operations are tracked as `PHASE6-GAP-STREAM-PRODUCERS` until the
follow-up streams and filesystem prompts implement them.
