# Performance Cranelift Big-Wins Addendum
...
addendum starts from the Performance counter and benchmark surface and keeps the
interpreter as the source of truth. Cranelift remains optional, feature-gated,
and default-off.

## Start Prerequisites

Foundation through Standard library are present in the repository and remain covered by the
existing layer gates. The Performance implementation is also present beyond the
minimum addendum start point:

| Area | Current status | Evidence |
| --- | --- | --- |
| Foundation-6 gates and docs | Present | `justfile`, `docs/stdlib-final-audit.md`, `docs/stdlib-known-gaps.md` |
| Performance work items 07.00-07.07 | Present | `docs/performance-preflight.md`, `docs/performance-methodology.md`, `tests/fixtures/performance/perf_smoke/`, `scripts/performance/bench_matrix.py`, `crates/php_vm/src/counters.rs` |
| Performance benchmark smoke | Present | `just benchmark-smoke` writes `target/performance/benchmark-smoke.json` |
| Performance counter JSON | Present | `php-vm run --counters-json ...`, `crates/php_vm/src/counters.rs`, `crates/php_perf` |
| Full Performance default gate | Present | `just verify-performance` |

The repository already contains later general Performance performance work:
...
- `crates/php_vm`: VM counters, tiering policy, inline-cache counters,
  quickening counters, and the guarded Performance JIT experiment integration.
...
`just benchmark-smoke` and `just verify-performance` are already concrete gates,
not placeholders. `benchmark-smoke` emits deterministic benchmark JSON with
VM counters for the Performance smoke fixtures.
...
- `docs/cranelift-jit-report-schema.md` for the Work item.04 JIT counter
  and report schema.
- `docs/cranelift-clif-dump.md` for the Work item.08 standalone CLIF dump
  and verifier smoke.

## Missing Addendum Building Blocks

These Cranelift-addendum pieces do not exist yet and are intentionally tracked
as follow-up work for work items after 07.CL.00:
...
| `docs/performance-cranelift-abi.md` | Present |
...
| Cranelift constrained native backend | Present as `CraneliftNoExecBackend`; it still verifies CLIF for non-executable subsets and additionally compiles the Work item.12 constant-return native subset, the Work item.17 inline checked int add/sub/mul subset, and the Work item.18 simple branch/count-loop subset when native execution is explicitly enabled. Documented in `docs/cranelift-no-exec-backend.md` |
...
| Cranelift-specific script directory | Present: `scripts/performance/cranelift/jit_diff.py` |
| Cranelift fixture tree | Present for 07.CL.10 eligibility fixtures under `tests/fixtures/performance/cranelift/eligibility/`, 07.CL.12 constant-return native fixtures under `tests/fixtures/performance/cranelift/native/`, 07.CL.13 helper-call fixtures under `tests/fixtures/performance/cranelift/helper-call/`, 07.CL.14/07.CL.15 side-exit and blacklist fixtures under `tests/fixtures/performance/cranelift/side-exit/`, 07.CL.16/07.CL.17 int-arithmetic matrix fixtures under `tests/fixtures/performance/cranelift/int_arithmetic/`, 07.CL.18/07.CL.19 branch/count-loop benchmark fixtures under `tests/fixtures/performance/cranelift/loops/`, 07.CL.20 through 07.CL.22 packed-array and packed-foreach fixtures under `tests/fixtures/performance/cranelift/arrays/`, 07.CL.23 known-call fixtures under `tests/fixtures/performance/cranelift/known-calls/`, 07.CL.24 string-concat fixtures under `tests/fixtures/performance/cranelift/string-concat/`, 07.CL.25 property-load metadata fixtures under `tests/fixtures/performance/cranelift/property-load-metadata/`, 07.CL.26 property-load fast-path fixtures under `tests/fixtures/performance/cranelift/property-load/`, 07.CL.27 method-call metadata fixtures under `tests/fixtures/performance/cranelift/method-call-metadata/`, 07.CL.28 direct method-call fixtures under `tests/fixtures/performance/cranelift/method-call/`, and the 07.CL.33 focused lifecycle test `cranelift_native_handle_copy_survives_original_handle_drop` |
| `just verify-cranelift` | Present; runs `jit-cranelift-smoke`, `jit-cranelift-diff`, `jit-cranelift-bench-smoke`, `jit-cranelift-report`, and `cranelift-guard-report` |
| `just jit-cranelift-smoke` | Present |
| `just jit-cranelift-diff` | Present; compares all fixtures under `tests/fixtures/performance/cranelift/`, uses test-only `--jit-eager` for Cranelift execution rows, and writes `target/performance/cranelift/diff.json` |
| `just jit-cranelift-bench-smoke` | Present; validates the int-arithmetic big-win smoke matrix and writes `target/performance/cranelift/bench-smoke.json` |
| `just jit-cranelift-report` | Present; generates the local Cranelift big-win matrix at `target/performance/cranelift/big_wins_report.json` |
| `just dump-cranelift-clif` | Present |
| `just cranelift-guard-report` | Present; runs the Big-Win report first, analyzes `target/performance/cranelift/big_wins_report.json`, and writes `target/performance/cranelift/guard-report.json` plus `target/performance/cranelift/guard-report.txt` |
| `just jit-cranelift-disasm` | Present; optional local diagnostic for 07.CL.B that writes FunctionId/IR-fingerprint-linked CLIF and code-size descriptors under `target/performance/cranelift/disasm/`; documented in `docs/cranelift-disassembly-dumps.md` |
| `just jit-cranelift-fuzz-smoke` | Present; optional 07.CL.C deterministic eligible-IR smoke that generates int-only fixtures, compares `--jit=off` with eager Cranelift, requires zero side exits, and stores seeds in `target/performance/cranelift/fuzz-smoke.json` |
...
| Cranelift big-win report JSON | Generated by `just jit-cranelift-report`; `target/performance/cranelift/big_wins_report.json` must not be committed |

## Addendum Order

The addendum must proceed in this order, validating after each work item before
starting the next:
...
9. Optional `07.CL.A` through `07.CL.F`: AOT research, disassembly dumps,
   ...
   research (`docs/research/cranelift-polymorphic-ics.md`), and offline
   framework-like smokes (`docs/cranelift-framework-smokes.md`).

## Work item.12 Native Constant-Return Subset

Work item.12 introduces the first native Cranelift execution path, but keeps
...
- `tests/fixtures/performance/cranelift/native/return-42.php`
- `tests/fixtures/performance/cranelift/native/function-return-42.php`
...
## Work item.13 Helper-Call Int Add/Mul Subset

Work item.13 extends the native subset to simple integer arithmetic through
...
- `tests/fixtures/performance/cranelift/helper-call/add-params.php`
- `tests/fixtures/performance/cranelift/helper-call/add-mul-expression.php`
- `tests/fixtures/performance/cranelift/helper-call/overflow-add.php`

## Work item.14 Side-Exit ABI And Interpreter Resume

Work item.14 adds `docs/adr/0783-cranelift-side-exit-model.md` and the
...
The Work item.13 helper-call subset uses side exits for non-zero helper
statuses. Work item.17 uses the same structured side-exit machinery for
...
implemented in these work items; ambiguous live state remains a JIT ineligibility
condition.

The required side-exit fixture is:

- `tests/fixtures/performance/cranelift/side-exit/helper-status-overflow.php`
...
## Work item.15 Guard-Failure Blacklisting

Work item.15 adds a process-local JIT blacklist policy for unstable compiled
...
- `tests/fixtures/performance/cranelift/side-exit/unstable-type-switch.php`
...
## Work item.16 Int-Arithmetic Benchmark Matrix

Work item.16 adds the first benchmarkable Cranelift big-win matrix for
integer leaf functions. The fixture family is:

- `tests/fixtures/performance/cranelift/int_arithmetic/repeated-function-call-add.php`
- `tests/fixtures/performance/cranelift/int_arithmetic/arithmetic-expression-chain.php`
- `tests/fixtures/performance/cranelift/int_arithmetic/negative-ints.php`
- `tests/fixtures/performance/cranelift/int_arithmetic/boundary-ints.php`
- `tests/fixtures/performance/cranelift/int_arithmetic/counted-loop-accumulator.php`
- `tests/fixtures/performance/cranelift/int_arithmetic/overflow-correctness.php`

`scripts/performance/cranelift/jit_bench_matrix.py` runs every scenario with
...
It writes `target/performance/cranelift/big_wins_report.json`. The initial checked-in
summary table lives in `docs/performance-cranelift-results.md`; generated JSON under
`target/` remains local output and is not committed.

## Work item.17 Inline Int Fast Path

Work item.17 replaces the executable helper-call path for eligible integer
...
## Work item.18 Simple Branches And Counted Loops

Work item.18 extends native Cranelift execution from single-block integer
...
- `tests/fixtures/performance/cranelift/loops/factorial-like-loop.php`
- `tests/fixtures/performance/cranelift/loops/sum-1-to-n.php`
- `tests/fixtures/performance/cranelift/loops/branchy-max-min.php`
- `tests/fixtures/performance/cranelift/loops/non-eligible-loop-call.php`
...
## Work item.19 Int Loop Benchmarks

Work item.19 stabilizes the numeric loop benchmark surface without adding a
CI-hard speedup threshold. The Cranelift matrix now includes explicit rows for:

- `repeated_int_function_calls` from
  `tests/fixtures/performance/cranelift/int_arithmetic/repeated-function-call-add.php`
- `sum_to_n` from `tests/fixtures/performance/cranelift/loops/sum-1-to-n.php`
- `fib_iterative` from `tests/fixtures/performance/cranelift/loops/fib-iterative.php`
- `branchy_int_loop` from
  `tests/fixtures/performance/cranelift/loops/branchy-int-loop.php`
...
JSON and `docs/performance-cranelift-results.md` report compile time, execution time,
total time, side exits, and code bytes separately. `iai-callgrind` is not a
workspace dependency; the existing Performance Valgrind/Callgrind smoke remains a
separate skip-safe gate, so this work item uses JSON counters and advisory
wall-clock totals.

## Work item.20 Packed Array Layout ABI

Work item.20 prepares read-only packed-array fast paths without adding
native array lowering yet. `docs/performance-cranelift-abi.md` documents the current
...
The work item adds packed and mixed array fixtures:
...
At this work item those fixtures validate output parity through fallback. work item
07.CL.21 owns the first packed-array fetch fast path and counters.

## Work item.21 Packed Array Int-Index Fetch Fast Path

Work item.21 adds the first executable packed-array path for the shape:
...
## Work item.22 Packed Foreach Int-Sum Fast Path

Work item.22 adds the first executable packed-foreach reduction path for
the canonical shape:
...
## Work item.23 Known Internal Calls

Work item.23 adds conservative helper-backed native fast paths for exactly
...
The Work item.23 fixtures are:

- `tests/fixtures/performance/cranelift/known-calls/strlen-valid.php`
- `tests/fixtures/performance/cranelift/known-calls/strlen-non-string.php`
- `tests/fixtures/performance/cranelift/known-calls/count-packed.php`
- `tests/fixtures/performance/cranelift/known-calls/count-mixed.php`
- `tests/fixtures/performance/cranelift/known-calls/strlen-wrong-arity.php`
...
## Work item.24 Simple String-Concat Fast Path

Work item.24 adds a conservative helper-backed native fast path for exactly
...
The Work item.24 fixtures are:

- `tests/fixtures/performance/cranelift/string-concat/two-strings.php`
- `tests/fixtures/performance/cranelift/string-concat/empty-strings.php`
- `tests/fixtures/performance/cranelift/string-concat/large-strings.php`
- `tests/fixtures/performance/cranelift/string-concat/template-loop.php`
- `tests/fixtures/performance/cranelift/string-concat/string-int-slow.php`
- `tests/fixtures/performance/cranelift/string-concat/object-to-string-slow.php`
...
## Work item.25 Monomorphic Property-Load Metadata

Work item.25 adds metadata collection for future monomorphic property-load
...
Work item.25 intentionally leaves actual Cranelift property-load lowering to
Work item.26. The interpreter remains the only execution path for property
fetches in this work item.

The Work item.25 fixtures are:

- `tests/fixtures/performance/cranelift/property-load-metadata/simple-declared-property.php`
- `tests/fixtures/performance/cranelift/property-load-metadata/subclass-layout-change.php`
- `tests/fixtures/performance/cranelift/property-load-metadata/magic-get.php`
- `tests/fixtures/performance/cranelift/property-load-metadata/property-hook.php`
- `tests/fixtures/performance/cranelift/property-load-metadata/uninitialized-typed-property.php`
...
## Work item.26 Monomorphic Property-Load Fast Path

Work item.26 adds the first helper-assisted native fast path for simple
...
The Work item.26 fixtures are:

- `tests/fixtures/performance/cranelift/property-load/simple-dto-property-read.php`
- `tests/fixtures/performance/cranelift/property-load/repeated-property-read-loop.php`
- `tests/fixtures/performance/cranelift/property-load/wrong-class-side-exit.php`
- `tests/fixtures/performance/cranelift/property-load/hook-magic-fallback.php`
- `tests/fixtures/performance/cranelift/property-load/uninitialized-error-path.php`
...
Work item.26 validation compared 58 diff fixtures and produced benchmark
...
## Work item.27 Monomorphic Method-Call Metadata

Work item.27 adds metadata collection for future monomorphic method-call
...
Work item.27 intentionally leaves direct native method calls to later work.
...
The Work item.27 fixtures are:

- `tests/fixtures/performance/cranelift/method-call-metadata/final-method.php`
- `tests/fixtures/performance/cranelift/method-call-metadata/normal-method-monomorphic.php`
- `tests/fixtures/performance/cranelift/method-call-metadata/subclass-override.php`
- `tests/fixtures/performance/cranelift/method-call-metadata/magic-call.php`
- `tests/fixtures/performance/cranelift/method-call-metadata/by-ref-arg-non-eligible.php`
...
## Work item.28 Direct Monomorphic Method-Call Fast Path

Work item.28 enables the first direct monomorphic instance-method fast path
for `--jit=cranelift`. The path reuses the Work item.27 method-call
...
fall back to the VM call path for non-native callees. The work item intentionally
...
The Work item.28 fixtures are:

- `tests/fixtures/performance/cranelift/method-call/repeated-small-method.php`
- `tests/fixtures/performance/cranelift/method-call/subclass-side-exit.php`
- `tests/fixtures/performance/cranelift/method-call/magic-fallback.php`
- `tests/fixtures/performance/cranelift/method-call/exception-in-callee.php`
- `tests/fixtures/performance/cranelift/method-call/service-dto-loop.php`
...
## Work item.29 Tiering Policy From Counters

Work item.29 connects eligibility, counters, guard failures, and native
...
budget tiering counters. `scripts/performance/cranelift/jit_bench_matrix.py`
...
## Work item.30 Process-Local JIT Compile Cache

Work item.30 adds a VM-owned process-local cache for Cranelift compiled
...
invalidation. The required Work item.30 gates remain
`jit-cranelift-diff`, `jit-cranelift-bench-smoke`, and the workspace
`jit-cranelift` test run.

## Work item.31 Consolidated Big-Win Matrix

Work item.31 promotes `scripts/performance/cranelift/jit_bench_matrix.py` from a
flat benchmark list to a consolidated Big-Win matrix report. The generated
`target/performance/cranelift/big_wins_report.json` uses schema version 2 and records
...
The script validates the Work item.31 structure before writing a passing
...
The accepted Work item.31 command sequence is:
...
## Work item.32 Guard Report And Minimization Hook

Work item.32 adds `scripts/performance/cranelift/guard_failure_report.py`. The
...
`target/performance/cranelift/big_wins_report.json`, then writes:
...
`scripts/minimize_runtime_failure.py` is present, the report includes commands
that write only under `target/performance/cranelift/minimized/`; the guard-report
tool itself does not mutate PHP fixtures.

The accepted Work item.32 validation commands are:
...
## Work item.33 Through 07.CL.35 Closure Inputs

Work item.33 audits the native Cranelift safety boundary in
`docs/safety-audit-cranelift.md` and
...
Work item.34 hardens local and CI usage. Cranelift commands first run
`scripts/performance/cranelift/platform_check.py`, which writes
`target/performance/cranelift/platform.json` with a machine-readable `pass` or
`skip` status. `.github/workflows/ci.yml` keeps the default Performance job
without Cranelift and adds a separate optional `jit-cranelift` job.

Work item.35 records the result decision matrix in
`docs/performance-cranelift-results.md`. The current conclusion is deliberately
conservative:
...
- method-call dispatch and broader object/call/string shapes should be
  revisited after future runtime;
...
- unsupported platforms skip Cranelift addendum gates with
  `target/performance/cranelift/platform.json`;
- persistent native code cache, ObjectModule/AOT output, OSR, trace JIT,
  ...
  framework/vendor benchmark suites are not production paths in Performance;
...
Current report artifacts are generated under `target/performance/cranelift/` and
...
The work item-pack completion map is maintained in
`docs/performance-cranelift-completion-audit.md`.
...
- default-on Cranelift would be premature until future runtime resolves workload,
...
| Optimizer | Not replaced. Cranelift relies on existing IR/HIR pipeline and does not add optimizer semantics. | Original optimizer work items remain necessary. |
| Quickening | Supplements hotness/tiering evidence but does not replace interpreter quickening. | Original quickening work items remain necessary. |
| ICs | Supplements method/property metadata and JIT-side guard decisions. | Original inline-cache work items remain necessary for interpreter/runtime performance. |
| Safety audit | Supplements the general Performance safety audit with native Cranelift ABI, helper, and executable-memory boundaries. | General cache/JIT/adaptive safety audit remains necessary. |
| CI hardening | Supplements default Performance CI with optional Cranelift feature coverage and skip JSON. | General Performance CI hardening remains necessary. |
...
Work item.00 validation commands:
...
The first command proves that the Performance benchmark smoke can still produce
counter JSON. The second command proves the existing Performance default gate still
