# Phase 7 Array Fast Paths

Prompt 07.38 keeps the Phase 5 ordered-vector array representation and adds
...
`PhpArray` tracks `packed_len: Option<usize>` inside request-local array storage.
`Some(len)` means the entries are known to be integer keys `0..len` in insertion
order. `None` means mixed or unknown. The exact public packed facade still
validates keys when requested, so metadata is only used for fast-path decisions.
...
`packed_element_fast`, so a quickened read cannot pass on mixed or unknown shape.
`foreach` snapshot creation records a hit for packed arrays while keeping the
snapshot semantics and iteration order unchanged. `count($array)` records a hit
for non-recursive array counts before dispatching to the existing builtin.
...
VM tests cover packed append/read/foreach/count counters, packed-to-mixed
transitions, reference element preservation, and foreach mutation order.
