# Phase 7 Cranelift Results

Date: 2026-06-23.

Reference target: PHP 8.5.7 (`php-8.5.7`).

This document records the local Prompt 07.CL.16 through 07.CL.32 Cranelift big-win
...
## Prompt 07.CL.31 Consolidated Matrix
...
`target/phase7/cranelift/big_wins_report.json`. The report remains valid when
...
Coverage is enforced by `scripts/phase7/cranelift/jit_bench_matrix.py`: each
...
- optional minimizer commands that write only under
  `target/phase7/cranelift/minimized/`.

Local Prompt 07.CL.32 output from the current Big-Win report found
...
## Prompt 07.CL.35 Decision Matrix

Prompt 07.CL.35 uses the current schema-2 Big-Win report
(`target/phase7/cranelift/big_wins_report.json`) plus the guard report
(`target/phase7/cranelift/guard-report.json`) as the decision source. The table
...
- Revisit after Phase 8/9: method-call dispatch, conversion-sensitive string
...
## Prompt 07.CL.23 Known-Call Rows

The Prompt 07.CL.23 rows cover the conservative helper-backed fast path for
...
counters required by the prompt.
...
## Prompt 07.CL.24 String-Concat Rows

The Prompt 07.CL.24 rows cover the conservative helper-backed fast path for
...
- Prompt 07.CL.25 is metadata-only and therefore does not add Cranelift
  ...
  `tests/fixtures/phase7/cranelift/property-load-metadata/`.
- Prompt 07.CL.26 adds helper-assisted monomorphic property-load benchmark
  ...
  `tests/fixtures/phase7/cranelift/property-load/` and compared 58 fixtures
  locally.
- Prompt 07.CL.27 is metadata-only and therefore does not add direct native
  ...
  now includes `tests/fixtures/phase7/cranelift/method-call-metadata/` and
  compared 63 fixtures locally.
- Prompt 07.CL.28 adds the first direct monomorphic method-call dispatch-helper
  ...
  `tests/fixtures/phase7/cranelift/method-call/`, and adds the
...
- Prompt 07.CL.29 adds the conservative Cranelift tiering policy in
  ...
  compiles.
- Prompt 07.CL.30 adds process-local, non-persistent compiled-function handle
  ...
  invalidations.
- Prompt 07.CL.31 consolidates the Big-Win matrix into schema version 2. The
  ...
  a correctness gate.
- Prompt 07.CL.32 adds `scripts/phase7/cranelift/guard_failure_report.py`, which
...
- `overflow_correctness` compiles, detects overflow in native code, records an
  ...
  interpreter.
- `iai-callgrind` is not a workspace dependency. The existing Phase 7
