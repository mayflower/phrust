# Phase 5 Array Semantics

Phase 5 arrays expose PHP-compatible key normalization, append-index behavior,
...
in `docs/phase5-foreach-semantics.md`.
...
Phase 6 should treat `php_runtime::PhpArray`, `ArrayKey`, `ArrayEntry`, and the
...
Performance-sensitive Phase 6 work should focus on append-heavy arrays,
...
known gap for Phase 5. The current implementation uses `i64` keys and saturates
...
Platform differences are limited to integer-width behavior. The Phase 5 target
