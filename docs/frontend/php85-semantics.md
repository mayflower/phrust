...
Semantic frontend models PHP 8.5 syntax structurally. It records HIR shape and
...
callable syntax, but Semantic frontend does not check whether it is invokable and does
not call it.
...
`(void)` lowers to `HirExprKind::Cast { kind: "void", ... }` so later layers can
see the parsed shape. The pinned PHP reference rejects this source form with a
parse error, so Semantic frontend emits `E_PHP_INVALID_VOID_CAST` and Rust acceptance must
...
Clone-with syntax lowers to `HirExprKind::CloneWith`. Semantic frontend records the
...
Semantic frontend intentionally avoids producing closure objects, callable targets, zvals,
