...
Semantic frontend does not implement a query engine. It exposes query-shaped APIs so a
...
- `collect_declarations(root, database, module_id)` runs the current fused
  Semantic frontend declaration, name-resolution, HIR-lowering, and check pass.
...
These Semantic frontend data sets are stable for one file and can be cached by
`ParseCacheKey`:
...
Runtime and later layers must invalidate or recompute data that depends on
...
assuming Semantic frontend executed them.
