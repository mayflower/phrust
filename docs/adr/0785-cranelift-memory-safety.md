# ADR 0785: Cranelift Memory Safety Boundary
...
- Require runtime native-execution opt-in before native entries are compiled or
  invoked.
- Delegate executable-memory allocation and W^X transitions to Cranelift's JIT
  memory provider.
...
reclamation tests before broad native execution can be enabled by default.
