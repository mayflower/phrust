# ADR-0026: Phase 5 GC Skeleton and Root Tracking

## Status

Accepted for Phase 5 Prompts 31 and 32.

## Context

Phase 5 needs a safe foundation for PHP-like reference counting and later cycle
...
shape, so Prompt 31 introduces observation and candidate detection rather than
freeing storage.
...
Prompt 32 adds internal weak handles and a tracked-heap test hook:
...
- No `unsafe` blocks are required for the Prompt 31 skeleton.
- Cycle candidates are diagnostic metadata. Prompt 32 collection may only break
...
- Add a tracing collector that mutates live runtime values during normal VM
  execution. Rejected for Phase 5; mutation is limited to deterministic test
  hooks until public semantics are specified.

## Phase 6 Handoff

Phase 6 should turn the debug root model into public GC behavior only after
