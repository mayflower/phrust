...
expression evaluation requires runtime value semantics that are outside the
Semantic frontend layer.

## Decision

Semantic frontend validates whether expressions are allowed in constant-expression
...
- Full value compatibility remains a later runtime layer concern.
