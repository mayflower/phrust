...
Parser produces a lossless CST. Semantic analysis needs structured access to
declarations, statements, expressions, names, types, and attributes without
moving semantic rules into the parser.

## Decision

Semantic frontend introduces `php_ast` as typed, read-only views over `php_syntax` CST
