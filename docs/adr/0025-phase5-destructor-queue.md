# ADR-0025: Phase 5 Destructor Queue MVP

Status: accepted

Prompt 30 asks for ADR-009 coverage of destructor queue invariants. This
repository already uses ADR-0009 for Pratt expression parsing, so this ADR is
the Phase 5 destructor-queue record.
...
Phase 5 introduces a VM-owned `DestructorQueue` in execution state:
...
- Run destructors immediately on `unset()`. Rejected for Phase 5 because exact
  refcount and cycle-collector timing is not implemented.
- Ignore destructors until Phase 6. Rejected because object lifetime is visible
  enough to affect Phase 5 object, clone, exception, and shutdown fixtures.

## Phase 6 Handoff

Phase 6 must decide unset-time destruction, cycle-collection destruction,
