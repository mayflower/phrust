# Phase 7 Experimental Release Build Profiles

Prompt 07.B is intentionally optional. The default debug/dev workflow and
...
| PGO generate | `-C profile-generate=target/phase7/pgo-data` | Collects workload profile data | Profile data is host/workload-specific |
| PGO use | `-C profile-use=target/phase7/pgo.profdata` | Guides branch layout and inlining | Stale profile can degrade performance |

Do not commit `target/phase7/pgo-data`, `*.profraw`, `*.profdata`, or generated
benchmark reports.
...
```bash
nix develop -c just release-profile-plan-phase7
```
...
3. `scripts/phase7/bench_matrix.py` pointed at that release binary
4. `scripts/phase7/compare_perf_json.py` comparing baseline and candidate JSON

## CI Policy

LTO/PGO is not required in CI for Phase 7. A future release pipeline may add a
