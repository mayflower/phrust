...
Phase 4 uses a small register-based IR as the executable contract between the
Phase 3 semantic frontend and the Rust VM. The IR defines stable IDs, units,
...
- `source_map`: source-map entries from IR targets back to Phase 3 origins.
...
The Phase 4 instruction set is:
...
Prompt 10 lowering recognizes Phase 3 HIR literals for `null`, booleans,
...
instructions, and terminators back to Phase 3 origin labels such as
...
Prompt 25 adds `Include { dst, kind, path }`, where `kind` is one of
...
`php_ir::lower_frontend_result()` consumes a Phase 3 `FrontendResult` and
produces a minimal top-level `IrFunction`. Prompt 10 lowers top-level scalar
...
Prompt 11 adds deterministic local slots to `IrFunction.locals`. Variable names
...
Prompt 12 lowers executable top-level namespace statement items recursively
...
Prompt 13 adds explicit `JumpIf` terminators for control-flow forms where both
...
Prompt 14 lowers named function declarations into separate `IrFunction`s with
...
Prompt 15 records required/optional parameters, folded constant defaults,
...
exists. Omitted optional parameters use only folded Phase 3 constant-expression
...
Prompt 16 lowers closure and arrow-function expressions into synthesized
...
Phase 3 explicit `use ($x)` metadata. Arrow functions derive by-value captures
...
Phase 3 semantic checks and does not execute PHP.
...
`php_ir::verify_unit()` checks the Prompt 04 structural invariants:
