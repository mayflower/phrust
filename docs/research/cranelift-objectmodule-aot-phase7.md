# Cranelift ObjectModule / AOT Research For Phase 7
...
Do not enable persistent native code caching in Phase 7. Keep Cranelift native
execution process-local, feature-gated, and default-off. Revisit
`ObjectModule` only after Phase 8/9 has a production lifecycle model for
...
| Area | `JITModule` in Phase 7 | `ObjectModule` future path |
| --- | --- | --- |
...
| Lifetime | Tied to the running process; Phase 7 leaks finalized modules for handle safety. | Tied to cache files, loader state, relocation records, and possibly worker lifetimes. |
...
`JITModule` is the right Phase 7 implementation vehicle because it lets the VM
...
`ObjectModule` becomes attractive only if later phases need a native-code cache
...
| Risk | Why Phase 7 does not accept it |
| --- | --- |
| Stale executable code | Phase 7 has no complete include/autoload/dependency model for persistent native code. |
...
The safe Phase 7 answer is to keep persistent native cache disabled and make
unsupported object-cache attempts fail closed.

## Prototype Status

No object file is generated by this prompt. The current dependency set includes
...
```text
target/phase7/cranelift/objectmodule/trivial_add.o
```
...
Phase 8/9 should revisit ObjectModule only if all of these are true:
