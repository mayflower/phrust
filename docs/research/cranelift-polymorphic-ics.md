# Cranelift Polymorphic Inline Cache Research for Performance
...
Performance already records property and method callsite profiles with three
...
`scripts/performance/cranelift/polymorphic_ic_experiment.py` generates local PHP
fixtures under `target/performance/cranelift/polymorphic-ic/fixtures/` and runs each
...
- `target/performance/cranelift/polymorphic-ic/report.json`;
- `target/performance/cranelift/polymorphic-ic/guard-report.json`;
- `target/performance/cranelift/polymorphic-ic/guard-report.txt`;
- generated fixtures and counter JSON under the same `target/` subtree.

The target is intentionally excluded from `just verify-cranelift`.
Framework-independent local fixtures are enough for this optional research
work item; making it a default gate would imply a runtime commitment Performance does
not make.

## Recommendation

Do not enable polymorphic JIT inline caches by default in Performance. Keep the
existing monomorphic fast paths as the executable subset and use this report to
size a future implementation. A later layer can revisit polymorphic Cranelift
guards after it has:
