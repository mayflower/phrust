...
| --- | --- | ---: | --- | --- |
| Dispatch | `instructions_executed` | 3372 | `tests/fixtures/performance/perf_smoke/array_fast_paths_v2.php` (870) | complete_for_current_counter_set |
| Calls | `function_calls, method_calls` | 88 | `tests/fixtures/performance/perf_smoke/stdlib_dispatch.php` (56) | complete_for_current_counter_set |
...
| Strings | `string_concats, concat_prealloc_hits, string_allocations` | 762 | `tests/fixtures/performance/perf_smoke/output_batching_v2.php` (109) | complete_for_current_counter_set |
| Output | `output_bytes, output_buffer_appends, output_buffer_batch_writes, output_batched_appends, output_batch_bytes, output_buffer_flushes, output_fast_appends` | 1394 | `tests/fixtures/performance/perf_smoke/array_fast_paths_v2.php` (334) | complete_for_current_counter_set |
...
| 4 | Strings | 109 counted event(s) in tests/fixtures/performance/perf_smoke/output_batching_v2.php | string concatenation allocation and conversion fast paths | PHP scalar conversion and binary-safe string behavior must stay exact | concat, scalar conversion, encoding-neutral, and error-order fixtures | high |
| 5 | Calls | 56 counted event(s) in tests/fixtures/performance/perf_smoke/stdlib_dispatch.php | call frame setup, method lookup, and later inline caches | call semantics include references, late static binding, visibility, and argument coercion | function, method, reference, variadic, and visibility fixtures | high |
| 6 | Standard Library Calls | 56 internal dispatch(es), 51 dispatch-cache hit(s), and 5 miss(es) in tests/fixtures/performance/perf_smoke/stdlib_dispatch.php covering count, strlen, is_int, array_values, strtolower | builtin dispatch and standard-library call shims | cache must not bypass named-argument conversion, arity checks, TypeError/ValueError diagnostics, or reflection metadata | per-builtin fixtures plus differential stdlib gates before any fast path | high |
