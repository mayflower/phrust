#!/usr/bin/env python3
"""Opt-in local Composer project smoke for Phase 5."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--out", type=Path, default=ROOT / "target/phase5/composer-smoke")
...
def write_report(out: Path, report: dict[str, object]) -> None:
    ...
    (out / "phase5-composer-smoke-report.json").write_text(json.dumps(report, indent=2), encoding="utf-8")
...
def main() -> int:
    ...
    print(
        f"[ok] Phase 5 composer smoke: total={len(results)} pass={passes} "
