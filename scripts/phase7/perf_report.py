#!/usr/bin/env python3
"""Render a human-readable Phase 7 performance report."""
...
DEFAULT_BENCHMARK = ROOT / "target/phase7/bench-phase7-smoke.json"
DEFAULT_BASELINE = ROOT / "target/phase7/baseline.json"
DEFAULT_COMPARE = ROOT / "target/phase7/perf-compare.md"
DEFAULT_FRAMEWORK_SMOKE = ROOT / "target/phase7/framework-smoke/summary.json"
DEFAULT_OUT = ROOT / "target/phase7/perf-report.md"
DEFAULT_JSON_OUT = ROOT / "target/phase7/perf-report.json"
KNOWN_GAPS = ROOT / "docs/known-gaps-phase7.md"
...
def gap_rows(path: Path) -> list[list[str]]:
    ...
    for line in text.splitlines():
        if not line.startswith("| PHASE7-GAP-"):
            continue
...
def render_markdown(summary: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "",
        "This report summarizes local Phase 7 benchmark JSON and VM counters.",
    ...
    if summary["benchmark_error"]:
        lines.extend(
            [
                ...
                "Run `nix develop -c just bench-phase7-smoke` or "
    ...
    if isinstance(framework, dict) and framework.get("data"):
        ...
        lines.append(
            f"No framework smoke summary found. Run `nix develop -c just framework-smoke-phase7` "
    ...
    if summary["known_gaps"]:
        ...
        lines.append("No Phase 7 known gaps were listed.")
    lines.append("")
    return "\n".join(lines)
...
def run_self_test() -> int:
    report = {
        "schema_version": 1,
        "run_id": "phase7-report-self-test",
        "environment": {
        ...
        "measurements": [
            {
                "scenario": {
                    "id": "phase7.perf_smoke.rust-vm.loop",
                    "name": "loop",
    ...
    assert "Phase 7 Performance Report" in markdown
    assert "phase7.perf_smoke.rust-vm.loop" in markdown
    ...
    assert "bench-phase7-smoke" in missing_markdown
...
def main() -> int:
    ...
    if benchmark_error:
        print(
            f"[warn] phase7 perf report wrote {rel(args.out)} without benchmark "
            f"data: {benchmark_error}"
        )
    else:
        print(f"[pass] phase7 perf report wrote {rel(args.out)}")
    return 0
