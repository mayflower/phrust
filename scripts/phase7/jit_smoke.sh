...
OUT_DIR="target/phase7"
OUT_JSON="$OUT_DIR/jit-smoke.json"
FIXTURES_DIR="tests/fixtures/phase7/jit"
...
"$VM" run --jit=on --counters-json "$OUT_DIR/jit-on-counters.json" "$FIXTURES_DIR/int-leaf-hot-loop.php" >"$OUT_DIR/jit-on.out"
diff -u "$OUT_DIR/jit-off.out" "$OUT_DIR/jit-on.out"
"$VM" run --jit=on --counters-json "$OUT_DIR/jit-rejected-counters.json" "$FIXTURES_DIR/rejected-fallback.php" >"$OUT_DIR/jit-rejected.out"
printf '90\n' >"$OUT_DIR/jit-expected.out"
printf '32\n' >"$OUT_DIR/jit-rejected-expected.out"
diff -u "$OUT_DIR/jit-expected.out" "$OUT_DIR/jit-on.out"
...
hot = json.loads(Path("target/phase7/jit-on-counters.json").read_text())
rejected = json.loads(Path("target/phase7/jit-rejected-counters.json").read_text())
if hot.get("jit_compile_attempts", 0) <= 0:
    raise SystemExit("[fail] jit=on recorded no compile attempts")
if hot.get("jit_compiled", 0) <= 0:
    raise SystemExit("[fail] jit=on recorded no compiled functions")
if hot.get("jit_executed", 0) <= 0:
    raise SystemExit("[fail] jit=on recorded no executions")
...
printf '%s\n' '[pass] phase7 jit smoke: default-off API, Cranelift lowering, guarded int-leaf execution, A/B output, and fallback passed'
