#!/usr/bin/env python3
"""Build a Phase 7 hot-path inventory from VM counter benchmark reports."""
...
DEFAULT_REPORT = ROOT / "target/phase7/bench-phase7-smoke.json"
DEFAULT_JSON = ROOT / "target/phase7/hotpaths.json"
DEFAULT_MARKDOWN = ROOT / "docs/hotpaths-phase7.md"
...
STD_LIB_FIXTURES = {
    "phase7.perf_smoke.rust-vm.arrays_packed": ("count",),
    "phase7.perf_smoke.rust-vm.autoload_smoke": (
        "spl_autoload_register",
        "strtolower",
    ),
    "phase7.perf_smoke.rust-vm.stdlib_dispatch": (
        "count",
        "strlen",
        "is_int",
        "array_values",
    ...
    "phase7.perf_smoke.rust-vm.strings_concat": ("strlen",),
}
...
def build_inventory(report: dict[str, Any], report_path: Path) -> dict[str, Any]:
    ...
    if not measurements:
        raise SystemExit(
            ...
            "run `just bench-phase7-smoke` with counters enabled"
        )
    ...
    for spec in CATEGORIES:
        ...
        if total > 0:
            ...
            gaps.append(
                {
                    "id": f"PHASE7-GAP-HOTPATH-{spec.key.upper()}-NO-EVENTS",
                    "description": (
                        f"No Phase 7 smoke fixture currently emits {spec.title} counter events; "
    ...
    if stdlib_rows:
        ...
        candidates.append(
            {
                "id": "phase7.hotpath.stdlib_calls",
    ...
    gaps.append(
        {
            "id": "PHASE7-GAP-HOTPATH-CORPUS-REPRESENTATIVENESS",
        ...
        "measurements": [
            {
                "engine": "rust-vm",
                "scenario": {
                    "id": "phase7.perf_smoke.rust-vm.function_calls",
...
def main() -> int:
    ...
    print(f"[pass] phase7 hotpath inventory wrote {rel(args.json_out)} and {rel(args.markdown_out)}")
    return 0
