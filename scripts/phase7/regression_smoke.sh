...
FIXTURES_DIR="tests/fixtures/phase7/regressions"
OUT_DIR="target/phase7/regression-smoke"
...
if [ ! -d "$FIXTURES_DIR" ]; then
    printf '[fail] missing Phase 7 regression fixture directory: %s\n' "$FIXTURES_DIR" >&2
    exit 1
fi
    ...
    for opt_level in 0 1 2; do
        for quickening in off on; do
            for inline_caches in off on; do
                ...
                if [ "$status" -ne 0 ]; then
                    printf '[fail] Phase 7 regression exited nonzero: %s (%s), status %s\n' "$fixture" "$label" "$status" >&2
                ...
                if ! cmp -s "$expected" "$stdout_norm"; then
                    printf '[fail] Phase 7 regression stdout mismatch: %s (%s)\n' "$fixture" "$label" >&2
                    exit 1
                fi
                if [ -z "$baseline_stderr" ]; then
                    baseline_stderr="$stderr_norm"
                elif ! cmp -s "$baseline_stderr" "$stderr_norm"; then
                    printf '[fail] Phase 7 regression stderr diverged: %s (%s)\n' "$fixture" "$label" >&2
                    exit 1
                fi
...
if [ "$fixture_count" -lt 8 ]; then
    printf '[fail] expected at least 8 Phase 7 regression fixtures, found %s\n' "$fixture_count" >&2
    exit 1
fi

printf '[pass] phase7 regression smoke compared %s fixture(s), %s run(s)\n' "$fixture_count" "$run_count"
