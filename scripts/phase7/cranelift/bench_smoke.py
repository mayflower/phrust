#!/usr/bin/env python3
"""Deterministic smoke gate for Phase 7 Cranelift helper-call reporting."""
...
DEFAULT_OUT = ROOT / "target/phase7/cranelift/bench-smoke.json"

SAMPLES = (
    {
        "name": "add_params",
        "fixture": ROOT / "tests/fixtures/phase7/cranelift/helper-call/add-params.php",
        "expect_helper_calls": 1,
        "expect_native_execution": True,
    },
    {
        "name": "add_mul_expression",
        "fixture": ROOT / "tests/fixtures/phase7/cranelift/helper-call/add-mul-expression.php",
        "expect_helper_calls": 2,
        "expect_native_execution": True,
    },
    {
        "name": "overflow_add",
        "fixture": ROOT / "tests/fixtures/phase7/cranelift/helper-call/overflow-add.php",
    ...
    env.update(
        {
            ...
            "TMP": str(tmp_dir),
            "TEMP": str(tmp_dir),
            "PHRUST_RANDOM_SEED": "phase7-cranelift-bench-smoke",
            "RUST_TEST_SEED": "phase7-cranelift-bench-smoke",
        }
    )
    return env
...
def main() -> int:
    ...
    for sample in SAMPLES:
        ...
        off = run_engine(
            ...
            fixture,
            "off",
            ROOT / "target/phase7/cranelift/bench-smoke-tmp" / sample["name"] / "off",
            args.timeout,
        )
        cranelift = run_engine(
            ...
            fixture,
            "cranelift",
            ROOT / "target/phase7/cranelift/bench-smoke-tmp" / sample["name"] / "cranelift",
            args.timeout,
        )
        require(
