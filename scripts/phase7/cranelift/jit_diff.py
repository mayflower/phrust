#!/usr/bin/env python3
"""Differential harness for Phase 7 Cranelift JIT modes."""
...
DEFAULT_FIXTURES = ROOT / "tests/fixtures/phase7/cranelift"
DEFAULT_OUT = ROOT / "target/phase7/cranelift/diff.json"
DEFAULT_DIFF_DIR = ROOT / "target/phase7/cranelift/diff"
    ...
    env.update(
        {
            ...
            "TMP": str(tmp_dir),
            "TEMP": str(tmp_dir),
            "PHRUST_RANDOM_SEED": "phase7-cranelift-jit-diff",
            "RUST_TEST_SEED": "phase7-cranelift-jit-diff",
        }
    )
    return env
...
def main() -> int:
    ...
    for fixture in fixtures:
        ...
        off = run_sample(
            ...
            fixture=fixture,
            mode="off",
            tmp_dir=ROOT / "target/phase7/cranelift/tmp" / fixture_key / "off",
        ...
        cranelift = run_sample(
            ...
            fixture=fixture,
            mode="cranelift",
            tmp_dir=ROOT / "target/phase7/cranelift/tmp" / fixture_key / "cranelift",
            timeout=args.timeout,
        )
        ...
        if reference is not None:
            reference_sample = run_sample(
                ...
                fixture=fixture,
                mode="reference",
                tmp_dir=ROOT / "target/phase7/cranelift/tmp" / fixture_key / "reference",
                timeout=args.timeout,
            )
