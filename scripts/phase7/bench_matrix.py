#!/usr/bin/env python3
"""Run deterministic Phase 7 benchmark smoke scenarios and emit JSON."""
...
DEFAULT_FIXTURES = ROOT / "tests/fixtures/phase7/perf_smoke"
DEFAULT_ENGINE = ROOT / "target/debug/php-vm"
DEFAULT_OUT = ROOT / "target/phase7/bench-phase7-smoke.json"
DEFAULT_REFERENCE = ROOT / "third_party/php-src/sapi/cli/php"
RUN_ID = "phase7-bench-matrix"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--repetitions", type=positive_int, default=int(os.getenv("PHRUST_PHASE7_BENCH_REPETITIONS", "1")))
    parser.add_argument("--warmups", type=positive_int, default=int(os.getenv("PHRUST_PHASE7_BENCH_WARMUPS", "0")))
    parser.add_argument("--timeout", type=float, default=float(os.getenv("PHRUST_PHASE7_BENCH_TIMEOUT", "5.0")))
    parser.add_argument(
        "--no-counters",
        action="store_true",
    ...
    env.update(
        {
            ...
            "TMP": str(tmp_dir),
            "TEMP": str(tmp_dir),
            "PHRUST_RANDOM_SEED": "phase7-bench-matrix",
            "RUST_TEST_SEED": "phase7-bench-matrix",
        }
    )
    return env
...
def measure_fixture(
    ...
    tmp_base = ROOT / "target/phase7/tmp" / engine.key / scenario_key
    ...
    for index in range(max(repetitions, 1)):
        ...
        if collect_counters and engine.key == "rust-vm":
            counters_path = (
                ROOT / "target/phase7/counters" / scenario_key / f"repeat-{index}.json"
            )
        samples.append(
    ...
    if collect_counters and engine.key == "rust-vm":
        display_counters_path = ROOT / "target/phase7/counters" / scenario_key / "repeat-N.json"
    measurement = {
        "scenario": {
            "id": f"phase7.perf_smoke.{engine.key}.{scenario_key}",
...
def report_environment(args: argparse.Namespace, reference_skip: str | None) -> dict[str, Any]:
    extra = {
        "tz": "UTC",
        "lc_all": "C",
        "tmp_root": "target/phase7/tmp",
        "deterministic_seed": "phase7-bench-matrix",
...
def main() -> int:
    ...
    if failed:
        print(f"[fail] phase7 benchmark smoke wrote {rel(args.out)}", file=sys.stderr)
        return 1
    print(f"[pass] phase7 benchmark smoke wrote {rel(args.out)}")
    return 0
