#!/usr/bin/env python3
"""Differential harness for Phase 7 optimizer levels."""
...
DEFAULT_OUT_DIR = ROOT / "target/phase7/optimizer-diff"
LEVELS = ("0", "1", "2")
DIAGNOSTIC_ID_RE = re.compile(r"E_PHP_[A-Z0-9_]+")

SELECTED_PHASE4_6_FIXTURES = (
    ...
    "tests/fixtures/phase6/_harness/json-pcre-date/json_basics.php",
    "tests/fixtures/phase6/_harness/stdlib/string_transform.php",
    "tests/fixtures/phase6/corpus/array_manipulation.php",
)
...
def fixture_paths() -> list[str]:
    ...
    optimizer_dir = ROOT / "tests/fixtures/phase7/optimizer"
    if optimizer_dir.is_dir():
        fixtures.update(rel(path) for path in optimizer_dir.rglob("*.php") if path.is_file())

    phase7_dir = ROOT / "tests/fixtures/phase7"
    fixtures.update(
        rel(path)
        for path in phase7_dir.rglob("*.php")
        if path.is_file() and path.with_name(path.name + ".out").is_file()
    )

    for fixture in SELECTED_PHASE4_6_FIXTURES:
        path = ROOT / fixture
        if not path.is_file():
