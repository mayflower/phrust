...
[[ -x "$PHP_REF_DIR/sapi/cli/php" ]] || fail \
  "reference PHP CLI is not built: $PHP_REF_DIR/sapi/cli/php"
php_version="$("$PHP_REF_DIR/sapi/cli/php" -r 'echo PHP_VERSION;')"
if [[ "$php_version" != "8.5.7" ]]; then
  fail "reference PHP CLI version is $php_version, expected exactly 8.5.7"
fi
...
request_parser_available="$("$PHP_REF_DIR/sapi/cli/php" -r 'var_export(function_exists("request_parse_body"));')"
if [[ "$request_parser_available" != "true" ]]; then
  fail "reference PHP CLI does not provide request_parse_body()"
fi
session_available="$("$PHP_REF_DIR/sapi/cli/php" -r 'var_export(extension_loaded("session"));')"
if [[ "$session_available" != "true" ]]; then
  fail "reference PHP CLI does not provide the session extension"
fi
printf '  cli: %s/sapi/cli/php (%s, tokenizer/request parser/session available)\n' \
