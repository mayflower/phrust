#!/usr/bin/env python3
"""Deterministic Phase 5 fuzz smoke for refs, COW arrays, and foreach."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--seed", type=int, default=int(os.getenv("PHRUST_PHASE5_FUZZ_SEED", "20260622")))
    parser.add_argument("--cases", type=int, default=int(os.getenv("PHRUST_PHASE5_FUZZ_CASES", "24")))
    parser.add_argument("--out", type=Path, default=ROOT / "target/phase5/fuzz-smoke")
...
def php_program(body: str) -> str:
    return (
        "<?php\n"
        "// phase5-runtime: category=regressions expect=pass "
        "regression_category=fuzz reference_behavior=generated fix_prompt=optional-a\n"
        f"{body}\n"
    )
...
def main() -> int:
    ...
    for index in range(args.cases):
        ...
        if not ok:
            ...
            if args.save_regressions:
                target = ROOT / "fixtures/phase5/regressions" / f"fuzz-{name}.php"
    ...
    (args.out / "phase5-fuzz-smoke-report.json").write_text(json.dumps(report, indent=2), encoding="utf-8")
    if failures:
        print(f"[fail] Phase 5 fuzz smoke: total={len(results)} fail={failures} path={args.out}")
        return 1
    print(f"[ok] Phase 5 fuzz smoke: total={len(results)} pass={len(results)} path={args.out}")
    return 0
