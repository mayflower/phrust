#!/usr/bin/env python3
"""Minimize a Phase 5 differential failure to a smaller PHP fixture."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--out", default="target/phase5/minimized.php", help="minimized output file")
...
def diff_signature(source_path: Path, rust_vm: Path, source: str) -> tuple[str, ...]:
    temp_path = Path("target/phase5/minimize-candidate.php")
    temp_path.parent.mkdir(parents=True, exist_ok=True)
    temp_path.write_text(source, encoding="utf-8")
    fixture = phase5_diff.Fixture(path=temp_path, category="ad_hoc", php_ref_required=True)
    reference = phase5_diff.run_reference(fixture)
    rust = phase5_diff.run_rust(fixture, rust_vm)
    if reference["status"] != "completed" or rust["status"] != "completed":
        return ()
    return tuple(phase5_diff.normalized_differences(reference, rust))
