#!/usr/bin/env python3
"""Phase 6 builtin/function differential harness."""
...
sys.path.insert(0, str(Path(__file__).resolve().parent / "phase6"))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--fixtures", default="tests/fixtures/phase6/_harness")
    parser.add_argument("--out", default="target/phase6/diff")
    parser.add_argument("--area", action="append", default=[])
    parser.add_argument("--file", action="append", default=[])
    parser.add_argument(
        "--vm-binary",
        default=os.environ.get("PHASE6_VM_CLI", ""),
...
def run(args: argparse.Namespace) -> dict:
    ...
    report_path = out_dir / "phase6-diff-report.json"
    report = {
...
def load_fixture(path: Path) -> Fixture:
    ...
    for line in path.read_text(encoding="utf-8", errors="replace").splitlines()[:10]:
        text = line.strip()
        if not text.startswith("// phase6-diff:"):
            continue
        for item in text[len("// phase6-diff:") :].strip().split():
            if "=" not in item:
                continue
