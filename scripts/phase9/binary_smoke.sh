...
case "$mode" in
  ...
  target)
    ...
    if [[ ! -x "$target_php" ]]; then
      printf '%s\n' '[todo] PHASE9_TARGET_CLI_COMPAT: Target PHP CLI compatibility binary is not built yet.'
      printf '%s\n' '[todo] Later Phase 9 CLI/SAPI work must provide -v and -r support before official PHPT execution.'
      exit 0
    fi
    version_out="$("$target_php" -v 2>&1 || true)"
    run_out="$("$target_php" -r 'echo "ok\n";' 2>&1 || true)"
    if [[ "$version_out" != *"PHP"* || "$run_out" != "ok" ]]; then
      printf '%s\n' '[todo] PHASE9_TARGET_CLI_COMPAT: Target PHP lacks required PHP CLI flags.'
    ...
    ;;
  *)
    printf 'unknown Phase 9 binary smoke mode: %s\n' "$mode" >&2
    exit 2
    ;;
esac
