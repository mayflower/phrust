#!/usr/bin/env python3
"""Generate Phase 6 function/class/constant coverage reports."""
...
OUT_DIR = REPO_ROOT / "target/phase6/function-coverage"
JSON_REPORT = OUT_DIR / "coverage.json"
DOC_REPORT = REPO_ROOT / "docs/function-coverage-phase6.md"
RUST_DUMP = REPO_ROOT / "target/debug/dump_phase6_registry"
EXTENSIONS = ["core", "standard", "json", "pcre", "date", "spl", "reflection", "tokenizer"]
KNOWN_GAP_BY_EXTENSION = {
    "standard": "PHASE6-GAP-STDLIB-FULL-PARITY",
    "json": "PHASE6-GAP-JSON-FLAGS-BYTE-PERFECT",
    "pcre": "PHASE6-GAP-PCRE-ADVANCED-FLAGS",
    "date": "PHASE6-GAP-DATE-TIMELIB-PARITY",
    "spl": "PHASE6-GAP-SPL-INTERFACE-METHOD-SURFACES",
    "reflection": "PHASE6-GAP-REFLECTION-INTERNAL-METHOD-SURFACE",
    "tokenizer": "PHASE6-GAP-TOKENIZER-ZEND-NUMERIC-IDS",
}
...
def main() -> int:
    ...
    try:
        ...
        print(f"phase6 function coverage error: {error}", file=sys.stderr)
        return 2
...
def load_rust_registry() -> dict[str, dict[str, Any]]:
    if not RUST_DUMP.is_file():
        raise FileNotFoundError(f"missing {RUST_DUMP}; build php_std dump-phase6-registry first")
...
def load_reference_symbols() -> tuple[dict[str, dict[str, Any]], dict[str, str]]:
    ...
    for kind, script in scripts.items():
        completed = subprocess.run(
            [str(php), str(REPO_ROOT / "scripts/phase6" / script), *EXTENSIONS],
            cwd=REPO_ROOT,
            check=True,
            text=True,
            stdout=subprocess.PIPE,
...
def build_report(
    ...
    return {
        ...
        "status_definitions": {
            ...
            "partial": "Class-like symbol is registered but Phase 6 metadata/behavior is intentionally partial.",
            ...
            "known_gap": "Symbol comparison is skipped or covered by an explicit Phase 6 known gap.",
        },
    }
