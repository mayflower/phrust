#!/usr/bin/env python3
"""Generate and run the Phase 6 selected extension PHPT smoke manifest."""
...
DEFAULT_MANIFEST = REPO_ROOT / "fixtures/phase6/phpt_extension_manifest.toml"
DEFAULT_PHP_SRC = REPO_ROOT / "third_party/php-src"
DEFAULT_OUT = REPO_ROOT / "target/phase6/phpt-extension-smoke"
...
def main() -> int:
    ...
    try:
        ...
        print(f"phase6 PHPT selector error: {error}", file=sys.stderr)
        return 2
    ...
    if not runner.is_file():
        print(
            f"phase6 PHPT selector error: missing runner {runner}; build php_testkit first",
            file=sys.stderr,
        )
        return 2
    ...
    print(f"[ok] phase6 PHPT selector report: {selector_report_path}")
...
def write_generated_allowlist(
    *,
    ...
    with allowlist_path.open("w", encoding="utf-8") as handle:
        handle.write("# Generated by scripts/phase6/phpt_extension_selector.py\n\n")
