#!/usr/bin/env python3
"""Generate optional performance Cranelift disassembly/code-size diagnostics."""
...
DEFAULT_OUT_DIR = ROOT / "target/performance/cranelift/disasm"
...
def normalized_env(tmp_dir: Path) -> dict[str, str]:
    ...
    env["PHRUST_RANDOM_SEED"] = "performance-cranelift-disasm-dump"
    env["RUST_TEST_SEED"] = "performance-cranelift-disasm-dump"
    return env
...
def disasm_text_for(
    *,
    ...
    lines = [
        "# performance Cranelift Disassembly Diagnostic",
        "",
        ...
        "",
        "Use the linked CLIF dump and code_bytes value for performance performance diagnostics.",
    ]
    return "\n".join(lines) + "\n"
...
def main() -> int:
    ...
    tmp_dir = ROOT / "target/performance/cranelift/tmp-disasm"
    ...
    for scenario in scenarios:
        ...
        entry = {
            ...
            "native_disassembly_status": "skipped",
            "native_disassembly_reason": "no object file or safe JIT-memory extraction path is available in performance",
            "objdump": objdump,
        }
    ...
    manifest = {
        "schema_version": 1,
        "status": status,
        "dump_kind": "performance-cranelift-code-size-and-clif",
