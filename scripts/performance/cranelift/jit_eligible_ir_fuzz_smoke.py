...
DEFAULT_OUT = ROOT / "target/performance/cranelift/fuzz-smoke.json"
DEFAULT_FIXTURE_DIR = ROOT / "target/performance/cranelift/fuzz/fixtures"
...
def env_for(seed: int) -> dict[str, str]:
    env = jit_bench_matrix.normalized_env(ROOT / "target/performance/cranelift/fuzz/tmp")
    env["PHRUST_RANDOM_SEED"] = f"performance-cranelift-fuzz-{seed}"
    env["RUST_TEST_SEED"] = f"performance-cranelift-fuzz-{seed}"
    return env
...
def generate_case(seed: int, index: int) -> GeneratedCase:
    ...
    name = f"perf_cl_fuzz_{seed:x}_{index}"
