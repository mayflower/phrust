#!/usr/bin/env python3
"""Offline framework-like Cranelift smokes for performance."""
...
DEFAULT_OUT = ROOT / "target/performance/cranelift/framework-smoke.json"
DEFAULT_FIXTURE_DIR = ROOT / "target/performance/cranelift/framework-smoke/fixtures"
...
function perf_framework_router_dispatch(string $path, int $id): int {
    $controller = new PerfFrameworkController();
    if ($path === "/show") {
...
$total = 0;
for ($i = 0; $i < 16; $i++) {
    $total += perf_framework_router_dispatch("/show", $i);
}
echo $total, "\\n";
""",
    },
...
function perf_framework_dto_score(PerfFrameworkDto $dto): int {
    return $dto->score;
}

$total = 0;
for ($i = 0; $i < 16; $i++) {
    $dto = new PerfFrameworkDto($i, $i + 3);
    $total += perf_framework_dto_score($dto);
}
echo $total, "\\n";
""",
    },
...
class PerfFrameworkService {
    public function score(PerfFrameworkServiceDto $dto, int $x): int {
        return $dto->base + $x + 1;
    }
}

$service = new PerfFrameworkService();
$dto = new PerfFrameworkServiceDto(7);
$total = 0;
for ($i = 0; $i < 24; $i++) {
...
""",
    },
    {
        ...
        "source": """<?php
function perf_framework_template_piece(string $lhs, string $rhs): string {
    return $lhs . $rhs;
}

$html = "";
for ($i = 0; $i < 24; $i++) {
    $html = perf_framework_template_piece($html, "<li>x</li>");
}
echo strlen($html), "\\n";
""",
    },
    {
        ...
        "source": """<?php
function perf_framework_config_read(array $config, int $index): int {
    return $config[$index];
}
...
$total = 0;
for ($i = 0; $i < 20; $i++) {
    $total += perf_framework_config_read($config, $i % 4);
}
echo $total, "\\n";
""",
    },
...
def normalized_env() -> dict[str, str]:
    env = jit_bench_matrix.normalized_env(ROOT / "target/performance/cranelift/framework-smoke/tmp")
    env["PHRUST_RANDOM_SEED"] = "performance-cranelift-framework-smoke"
    env["RUST_TEST_SEED"] = "performance-cranelift-framework-smoke"
    return env
