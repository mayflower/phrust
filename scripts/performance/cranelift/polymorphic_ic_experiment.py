#!/usr/bin/env python3
"""Local default-off polymorphic IC guard experiment for performance."""
...
DEFAULT_OUT = ROOT / "target/performance/cranelift/polymorphic-ic/report.json"
DEFAULT_FIXTURE_DIR = ROOT / "target/performance/cranelift/polymorphic-ic/fixtures"
DEFAULT_COUNTER_DIR = ROOT / "target/performance/cranelift/polymorphic-ic/counters"
MAX_POLYMORPHIC_ENTRIES = 4


FIXTURES: dict[str, str] = {
    "property_polymorphic": """<?php
class PerfPolyPropA { public int $value = 1; }
class PerfPolyPropB { public int $value = 2; }
class PerfPolyPropC { public int $value = 3; }
function perf_poly_prop_read($object): int { return $object->value; }
echo perf_poly_prop_read(new PerfPolyPropA());
echo perf_poly_prop_read(new PerfPolyPropB());
echo perf_poly_prop_read(new PerfPolyPropC());
""",
    "property_megamorphic": """<?php
class PerfMegaPropA { public int $value = 1; }
class PerfMegaPropB { public int $value = 2; }
class PerfMegaPropC { public int $value = 3; }
class PerfMegaPropD { public int $value = 4; }
class PerfMegaPropE { public int $value = 5; }
function perf_mega_prop_read($object): int { return $object->value; }
echo perf_mega_prop_read(new PerfMegaPropA());
echo perf_mega_prop_read(new PerfMegaPropB());
echo perf_mega_prop_read(new PerfMegaPropC());
echo perf_mega_prop_read(new PerfMegaPropD());
echo perf_mega_prop_read(new PerfMegaPropE());
""",
    "method_polymorphic": """<?php
class PerfPolyMethodA { public function value(): int { return 1; } }
class PerfPolyMethodB { public function value(): int { return 2; } }
class PerfPolyMethodC { public function value(): int { return 3; } }
function perf_poly_method_call($object): int { return $object->value(); }
echo perf_poly_method_call(new PerfPolyMethodA());
echo perf_poly_method_call(new PerfPolyMethodB());
echo perf_poly_method_call(new PerfPolyMethodC());
""",
    "method_megamorphic": """<?php
class PerfMegaMethodA { public function value(): int { return 1; } }
class PerfMegaMethodB { public function value(): int { return 2; } }
class PerfMegaMethodC { public function value(): int { return 3; } }
class PerfMegaMethodD { public function value(): int { return 4; } }
class PerfMegaMethodE { public function value(): int { return 5; } }
function perf_mega_method_call($object): int { return $object->value(); }
echo perf_mega_method_call(new PerfMegaMethodA());
echo perf_mega_method_call(new PerfMegaMethodB());
echo perf_mega_method_call(new PerfMegaMethodC());
echo perf_mega_method_call(new PerfMegaMethodD());
echo perf_mega_method_call(new PerfMegaMethodE());
""",
}
...
def normalized_env() -> dict[str, str]:
    env = jit_bench_matrix.normalized_env(ROOT / "target/performance/cranelift/polymorphic-ic/tmp")
    env["PHRUST_RANDOM_SEED"] = "performance-cranelift-polymorphic-ic"
    env["RUST_TEST_SEED"] = "performance-cranelift-polymorphic-ic"
    return env
