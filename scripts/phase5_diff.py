#!/usr/bin/env python3
"""Phase 5 runtime differential harness."""
...
def main() -> int:
    ...
    if report["summary"]["fail"]:
        print(
            f"[fail] Phase 5 diff failures={report['summary']['fail']} "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run Phase 5 PHP runtime fixtures against REFERENCE_PHP and php_vm_cli."
    )
    parser.add_argument("--fixtures", default="fixtures/phase5", help="Phase 5 fixture root")
    parser.add_argument("--out", default="target/phase5/diff", help="output directory")
    ...
    parser.add_argument(
        ...
        choices=CATEGORIES,
        default=[],
        help="Phase 5 category to select",
    )
...
def run(args: argparse.Namespace) -> dict:
    ...
    report_path = out_dir / "phase5-diff-report.json"
...
def load_fixture(path: Path, fixtures_root: Path) -> Fixture:
    ...
    for line in path.read_text(encoding="utf-8", errors="replace").splitlines()[:8]:
        text = line.strip()
        prefix = None
        for candidate in ("// phase5-runtime:", "# phase5-runtime:"):
...
def validate_fixture_metadata(fixture: Fixture) -> None:
    if fixture.category != "regressions":
        return
    required = ("regression_category", "reference_behavior", "fix_prompt")
    ...
    if fixture.expect == "known_gap":
        if not fixture.known_gap_id:
            return result(
                ...
                reference,
                rust,
                "known-gap fixture must declare phase5-runtime known_gap=<ID>",
            )
...
class HarnessError(Exception):
    """Phase 5 harness configuration error."""
