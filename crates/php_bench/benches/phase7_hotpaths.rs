...
function phase7_call($a, $b) {
    return $a + $b;
}

$sum = 0;
for ($i = 0; $i < 48; $i++) {
    $sum = phase7_call($sum, $i);
}
echo $sum;
"#;

const VM_PROPERTY_SOURCE: &str = r#"<?php
class Phase7PropertyBox {
    public $value = 3;
...
fn bench_frontend(c: &mut Criterion) {
    ...
    c.bench_function("phase7/lexer_parser_smoke", |b| {
        b.iter(|| {
    ...
    c.bench_function("phase7/ir_lower_smoke", |b| {
        b.iter(|| {
...
fn bench_vm(c: &mut Criterion) {
    ...
    c.bench_function("phase7/vm_dispatch_micro_loop", |b| {
        b.iter(|| execute_unit(black_box(&loop_unit)));
    });
    c.bench_function("phase7/function_call_dispatch", |b| {
        b.iter(|| execute_unit(black_box(&call_unit)));
    });
    c.bench_function("phase7/property_lookup", |b| {
...
fn bench_runtime(c: &mut Criterion) {
    ...
    c.bench_function("phase7/packed_array_access", |b| {
        b.iter(|| {
            let mut sum = 0i64;
            for index in 0..128 {
    ...
    c.bench_function("phase7/mixed_array_access", |b| {
        b.iter(|| {
    ...
    c.bench_function("phase7/string_concat_builder", |b| {
        b.iter(|| {
...
criterion_group! {
    name = phase7_hotpaths;
    config = configured_criterion();
    targets = bench_frontend, bench_vm, bench_runtime
}
criterion_main!(phase7_hotpaths);
