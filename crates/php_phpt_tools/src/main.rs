...
fn generate_module_tests<W: Write>(args: &[String], stdout: &mut W) -> Result<i32, String> {
    ...
    for entry in smoke_candidates {
        ...
        if runner::run_one_phpt(&reference_context, &entry.path, generated.len())?.outcome != "PASS"
        {
            continue;
        }
...
mod tests {
    ...
    fn rerun_manifest_keeps_only_non_green_paths() {
        ...
        let status = runner::rerun_manifest(
            &[
                "--results".to_string(),
    ...
    fn phpt_runner_reads_non_utf8_sources_lossily() {
        ...
        .unwrap();
        let source = runner::read_phpt_source_lossy(&path).unwrap();
        fs::remove_file(&path).unwrap();
