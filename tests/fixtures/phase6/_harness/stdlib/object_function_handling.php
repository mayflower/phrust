<?php
// phase6-diff: id=PHASE6_STDLIB_OBJECT_FUNCTION_HANDLING area=stdlib expect=pass
class Phase6Box {
    public $pub = 'P';
    protected $prot = 'R';
    private $priv = 'V';
...
function phase6_join_args($a, $b = 'D') {
    return $a . $b . ':' . func_num_args() . ':' . func_get_arg(0) . ':' . count(func_get_args());
}

function phase6_named_args($a, $b) {
    return $a . $b;
}

class Phase6CallTarget {
    public static function target($value) {
        return 'S' . $value;
    }

    public static function forward($value) {
        return forward_static_call(['Phase6CallTarget', 'target'], $value);
    }
}

$box = new Phase6Box();
$outside = get_object_vars($box);
$methods = get_class_methods('Phase6Box');
$classVars = get_class_vars('Phase6Box');

echo $outside['pub'], "\n";
echo array_key_exists('prot', $outside) ? "bad\n" : "no-prot\n";
echo $box->objectVars(), "\n";
echo $box->mangledVars(), "\n";
echo Phase6Box::classVars(), "\n";
...
echo call_user_func('phase6_join_args', 'A', 'B'), "\n";
echo call_user_func_array('phase6_named_args', ['b' => 'B', 'a' => 'A']), "\n";
echo call_user_func(['Phase6CallTarget', 'target'], 'X'), "\n";
echo Phase6CallTarget::forward('Y'), "\n";
